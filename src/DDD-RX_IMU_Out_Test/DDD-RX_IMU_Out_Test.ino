...
void stateDisarmed(){
  ...
  if(state_change_request){ stateChangeRequest(State::idle, STATE_DURATION*2); // Request state change
  }else{
 ...
 * In the case of deviding by 3 the max roll rate is aprox 166 degrees per second ( (500-deadband)/3 = 166d/s )
...
float pwmToAngularRate(int pwm){
  ...
  // midOffset = pwm - mid <-- mid id either upperMid or lowerMid depending on wich side pwm is
...
void printRxChannelLR(int rx_signal){
  if (rx_signal - (PWM_MID-PWM_DEADBAND) < 0)Serial.print("^^^");       //RCPWM - 1480 if Deadband is 20
  else if (rx_signal - (PWM_MID+PWM_DEADBAND) > 0)Serial.print("vvv");  //RCPWM - 1520 if Deadband is 20
  else Serial.print("-+-");
...
void printRxChannelUD(int rx_signal){
  if (rx_signal - (PWM_MID-PWM_DEADBAND) < 0)Serial.print("<<<");       //RCPWM - 1480 if Deadband is 20
  else if (rx_signal - (PWM_MID+PWM_DEADBAND) > 0)Serial.print(">>>");  //RCPWM - 1520 if Deadband is 20
  else Serial.print("-+-");
  Serial.print(rx_signal);
}
