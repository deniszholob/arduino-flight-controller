...
void setup_ThrottleSafety() {
  ...
  // boolean isRxOff = rx_pwm_signal[2] < (990); // signal should be 0 as its hasn't caused interrupt
  boolean isRxOff = rx_pwm_signal[2] < (1050);
  boolean isThrottleLow = rx_pwm_signal[3] < (1050);

  // Wait/Loop until the receiver is active and the throttle is set to the lower position.
  while (!(isRxOff && isThrottleLow)) {
