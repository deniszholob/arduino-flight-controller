...
ISR(PCINT0_vect) {
  ...
  for (int i = 0; i < RX_CHANNELS; i++) {
    boolean isPinHigh = PINB & pwm_input_pin[i];
    // Find start of PWM Signal (High)
    if (!rx_last_PWM_state[i] && isPinHigh) { // Did Input change from LOW(false) to HIGH(true) AND Is the input pin HIGH?
      rx_last_PWM_state[i] = true;            // Remember current input state (true/HIGH)
      // Set channel timer to current time
      rx_timer[i] = rx_timer_current;
    }
    // Find End of PWM signal (Low)
    else if (rx_last_PWM_state[i] && !isPinHigh) { // Did Input change from HIGH(true) to LOW(false) AND Is the input pin LOW?
      rx_last_PWM_state[i] = false;                // Remember current input state (false/LOW)
      // Channel microseconds value: current time - timer value (range should be 1000us - 2000us)
      rx_signal[i] = rx_timer_current - rx_timer[i];
    }
  }
}
...
void setup_ThrottleSafety() {
  boolean isRxOff = rx_signal[2] < (990);       // RX is not ON (thus signal should be 0 as its hasn't caused interrupt)
  boolean isThrottleOn = rx_signal[2] > (1020); // Throttle not off

  // Wait/Loop until the receiver is active and the throttle is set to the lower position.
  while (isRxOff || isThrottleOn) {
...
void printPWMValues() {
  Serial.print("Roll:");
  if (rx_signal[0] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("<<<"); //RCPWM - 1480 if Deadband is 20
  else if (rx_signal[0] - (PWM_MID + PWM_DEADBAND) > 0)
  ...
  Serial.print("  Pitch:");
  if (rx_signal[1] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("^^^"); //RCPWM - 1480 if Deadband is 20
  else if (rx_signal[1] - (PWM_MID + PWM_DEADBAND) > 0)
  ...
  if (rx_signal[2] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("vvv"); //RCPWM - 1480 if Deadband is 20
  else if (rx_signal[2] - (PWM_MID + PWM_DEADBAND) > 0)
    Serial.print("^^^"); //RCPWM - 1520 if Deadband is 20
  else
    Serial.print("-+-");
  Serial.print(rx_signal[2]);
  ...
  Serial.print("  Yaw:");
  if (rx_signal[3] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("<<<"); //RCPWM - 1480 if Deadband is 20
  else if (rx_signal[3] - (PWM_MID + PWM_DEADBAND) > 0)
    Serial.print(">>>"); //RCPWM - 1520 if Deadband is 20
  else
    Serial.print("-+-");
  Serial.println(rx_signal[3]);
