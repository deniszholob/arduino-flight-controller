...
ISR(PCINT0_vect) {
  ...
  for (int i = 0; i < RX_CHANNELS; i++) {
    ...
    if (!last_state[i] && isPinHigh) { // Did Input change from LOW(false) to HIGH(true) AND Is the input pin HIGH?
      last_state[i] = true;            // Remember current input state (true/HIGH)
      // Set channel timer to current time
      rx_timer[i + 1] = rx_timer[0];
    ...
    // Find End of PWM signal (Low)
    else if (last_state[i] && !isPinHigh) { // Did Input change from HIGH(true) to LOW(false) AND Is the input pin LOW?
      ...
      rx_signal[i] = rx_timer[0] - rx_timer[i + 1];
    }
  }
}
